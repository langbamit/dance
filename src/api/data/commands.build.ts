...
export async function build(builder: Builder) {
  ...
  const commandsDoc = parseYamlDocument(commandsDocSource),
    commandDocByName = Object.fromEntries(
      (commandsDoc.contents as YAMLMap<Scalar<string>, YAMLMap>).items.map(
        (item) => [item.key.value, item.value!],
      ),
    ),
    toRemove = new Set(Object.keys(commandDocByName));

  let anonymousCommandsDoc = commandsDoc.get("anonymous") as
    | YAMLSeq<YAMLMap>
    | undefined;
  ...
  const anonymousCommandByCommands = Object.fromEntries(
      anonymousCommandsDoc.items.map((item) => [
        item.get("commands", false) as string,
        item,
      ]),
    ),
    anonymousToRemove = Object.fromEntries(
      anonymousCommandsDoc.items.map((item, i) => [
        item.get("commands", false) as string,
        i,
      ]),
    );
  ...
  const allCommands = commandModules.flatMap((module) => [
    ...(module.additional as (Builder.AdditionalCommand & {
      doc: undefined;
    })[]),
  ...
  for (const command of allCommands) {
    ...
    if (
      command.title !==
      (existingCommandDoc.getIn(["title", "en"], false) as string)
    ) {
      if (command.title === undefined) {
    ...
    if (
      keys !== (existingCommandDoc.getIn(["keys", "qwerty"], false) as string)
    ) {
      if (keys === undefined) {
    ...
    if (
      command.commands !== (existingCommandDoc.get("commands", false) as string)
    ) {
      if (command.commands === undefined) {
  ...
  (commandsDoc.contents as YAMLMap.Parsed<Scalar.Parsed>).items.sort((a, b) =>
    (a.key.value as string).localeCompare(b.key.value as string),
  );

  // Sort anonymous commands by `commands` and `args`.
  anonymousCommandsDoc.items.sort((a, b) =>
    (a.get("commands", false) as string).localeCompare(
      b.get("commands", false) as string,
    ),
  );
  ...
  const contents = stringifyYaml(commandsDoc, { lineWidth: 100 }).replace(
