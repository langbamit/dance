...
export async function build(builder: Builder) {
  ...
  const [commands, languages, layouts, commandModules] = (await Promise.all([
    ...["commands.yaml", "languages.yaml", "layouts.yaml"].map(
      async (filename) => {
        const contents = await readFile(resolveData(filename), {
          encoding: "utf-8",
        });

        return parseYaml(contents, { schema: "core" }) as Record<
          string,
          object
        >;
      },
  ...
  await Promise.all(
    Object.entries(languages).flatMap(([languageId, language]) => {
      ...
      return language.layouts.map(
        async (layoutId) =>
          await writeFile(
            resolve(__dirname, "layouts", layoutId + suffix),
            unindent(12)`
            # Commands: \`${layoutId}\`
...
class Renderer {
  ...
  public readonly modulesAndCommands: (readonly [
    Builder.ParsedModule,
    readonly Command[],
  ])[];
  ...
  public constructor(
    ...
    const commandsByModuleName = new Map(
      modules.map((module) => [module.name, [] as Command[]]),
    );

    for (const [commandId, module] of moduleByCommandId) {
      commandsByModuleName
        .get(module.name)!
        .push(commands[commandId] ?? this.anonymousCommands[commandId]);
    }

    this.modulesAndCommands = [...commandsByModuleName].map(
      ([moduleName, commands]) =>
        [modules.find((x) => x.name === moduleName)!, commands] as const,
    );
  }
      ...
      ${this.modulesAndCommands
        .map(([module]) =>
          ...
          ${module.functions
            .map((f) =>
              unindent(14)`
              <a name="${module.name === "misc" ? "" : module.name + "."}${
                f.nameWithDot
              }" />

              ### [\`${module.name === "misc" ? "" : module.name + "."}${
                f.nameWithDot
              }\`](${commandsDir}/${module.name}.ts#L${f.line})

              ${
                sanitizeDoc(
                  this._resolveProp(
                    "doc",
                    this.commands[f.qualifiedName],
                    language,
                  )!,
                ) + "\n"
              }
              ${(() => {
                ...
                return supportedInputs.length === 0
                  ? ""
                  : "This command:" +
                      supportedInputs.map((x) => `\n- ${x}.`).join("") +
                      "\n";
              })()}
              ${
                "keys" in f.properties
                  ? `Default keybinding: ${this._resolveKeys(
                      this.commands[f.qualifiedName],
                      layout,
                    )}\n`
                  : ""
  ...
  private _renderQuickRefTable(
    ...
    const rows: string[][] = this.modulesAndCommands.flatMap(([module]) => {
      const modulePrefix = module.name === "misc" ? "" : module.name + ".",
        allCommands = [] as (
          | Builder.ParsedFunction
          | Builder.AdditionalCommand
        )[];
      ...
      return allCommands.map((f, i, { length }) => {
        const identifier =
            "name" in f ? modulePrefix + f.nameWithDot : f.qualifiedIdentifier,
          summary = "summary" in f ? f.summary : f.title,
          command =
            "summary" in f
              ? this.commands[f.qualifiedName]
              : this.commands[f.qualifiedIdentifier!] ??
                this.anonymousCommands[f.commands!],
        ...
        return [
          ...
          `<td>${keys
            .map(
              ({ key, when }) => `<code>${key}</code> (<code>${when}</code>)`,
            )
            .join("")}</td>`,
        ];
      });
...
function determineSupportedInputs(_f: Builder.ParsedFunction) {
  ...
  //     supported.push(
  //       `accepts a register (by default, it uses \`${match[1]}\`)`,
  //     );
  ...
  //   } else if (
  //     /^(Context|vscode.Text(Editor|Document))$/.test(type) ||
  //     name === "selections"
  //   ) {
...
function sanitizeDoc(doc: string) {
  ...
  doc = doc.replace(/^\|(.+?)\|$/gm, (line: string) =>
    line.replace(/(?<! )\|(?! )/g, "\\|"),
  );
