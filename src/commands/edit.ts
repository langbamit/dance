 ...
 *
 * @keys `s-r` (helix: normal), `s-r` (helix: select)
 *
 * @commands
 *
 * | Title                              | Identifier               | Keybinding                                       | Commands                                                                                                                       |
 * | ---------------------------------- | ------------------------ | ------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------ |
 * | Pick register and replace          | `selectRegister-insert`  | `c-r` (helix: insert)                            | `[".selectRegister", { +register }], [".edit.insert", { ... }]`                                                                |
 ...
 * | Paste before and select            | `paste.before.select`    | `s-p` (helix: normal)                            | `[".edit.insert", { handleNewLine: true, where: "start", shift: "select", ... }]`                                              |
 * | Paste after and select             | `paste.after.select`     | `p` (helix: normal)                              | `[".edit.insert", { handleNewLine: true, where: "end"  , shift: "select", ... }]`                                              |
 ...
 * | Paste all before and select        | `pasteAll.before.select` |                                                  | `[".edit.insert", { handleNewLine: true, where: "start", all: true, shift: "select", ... }]`                                   |
 * | Paste all after and select         | `pasteAll.after.select`  |                                                  | `[".edit.insert", { handleNewLine: true, where: "end"  , all: true, shift: "select", ... }]`                                   |
 * | Delete                             | `delete`                 | `a-d` (helix: normal), `a-d` (helix: select)     | `[".edit.insert", { register: "_", ... }]`                                                                                     |
 * | Delete and switch to Insert        | `delete-insert`          | `a-c` (helix: normal), `a-c` (helix: select)     | `[".modes.set", { mode: "insert", +mode }], [".edit.insert", { register: "_", ... }]`                                          |
 * | Copy and delete                    | `yank-delete`            | `d` (helix: normal), `d` (helix: select)         | `[".selections.saveText", { +register }],                                            [".edit.insert", { register: "_", ... }]` |
 * | Copy, delete and switch to Insert  | `yank-delete-insert`     | `c` (helix: normal), `c` (helix: select)         | `[".selections.saveText", { +register }], [".modes.set", { mode: "insert", +mode }], [".edit.insert", { register: "_", ... }]` |
 * | Copy and replace                   | `yank-replace`           |                                                  | `[".selections.saveText", { register: "tmp" }], [".edit.insert"], [".updateRegister", { copyFrom: "tmp", ... }]`               |
 */
export async function insert({
  ...
  text?: string;
  shift?: Shift;
  where?: "active" | "anchor" | "start" | "end" | undefined;
}>) {
  const register = getRegister("dquote", Register.Flags.CanRead);
  ...
  if (contents === undefined || contents.length === 0) {
    throw new Error(
      `register "${register.name}" does not contain any saved text`,
    );
  }
  ...
  if (all) {
    if (shift !== Shift.Select) {
      throw new ArgumentError(
        '`all` is only compatible with `shift: "select"`',
      );
    }
    ...
    for (const insertedRange of insertedRanges) {
      ...
      for (const content of contents) {
        const newSelection = Selections.fromLength(
          offset,
          content.length,
          false,
          document,
  ...
  if (!["active", "anchor", "start", "end"].includes(where)) {
    throw new Error(
      `"where" must be one of "active", "anchor", "start", "end", or undefined`,
    );
  }
 ...
 * Join lines.
 *
 * @keys `s-j` (helix: normal), `s-j` (helix: select)
 */
export function join({ separator }: CommandArguments<{ separator?: string }>) {
 ...
 *
 * @keys `s-a-j` (helix: normal), `s-a-j` (helix: select)
 */
export async function join_select({
  separator,
}: CommandArguments<{ separator?: string }>) {
 ...
 */
export function indent({ repetitions }: CommandArguments) {
 ...
 *
 * @keys `a->` (kakoune: normal), `a->` (kakoune: select)
 */
export function indent_withEmpty({ repetitions }: CommandArguments) {
 ...
 * @keys `a-<` (kakoune: normal), `a-<` (kakoune: select)
 */
export function deindent({ repetitions }: CommandArguments) {
  return deindentLines(
    Selections.lines(),
    repetitions,
    /* deindentIncomplete= */ false,
  );
}
 ...
 *
 * @keys `<` (helix: normal), `<` (helix: normal)
 */
export function deindent_withIncomplete({ repetitions }: CommandArguments) {
  return deindentLines(
    Selections.lines(),
    repetitions,
 ...
 * Swap case.
 *
 * @keys `` s-` `` (helix: normal), `` s-` `` (helix: select)
 */
export function case_swap() {
  return replace((text) => {
    let builtText = "";
 ...
 * Replace characters.
 *
 * @keys `r` (helix: normal), `r` (helix: select)
 */
export async function replaceCharacters({
  ...
  return _.run(() =>
    edit((editBuilder, selections, document) => {
      for (const selection of selections) {
        ...
        const lastLine = document
          .lineAt(i)
          .range.with(undefined, selection.end);
...
export function align({
  selections,
  fill = " ",
}: CommandArguments<{ fill?: string }>) {
  // let col = 0;
  // const column_widths = [];
  // const last_line = _.document.lineCount - 1;
  // for (const sel of selections) {
  //   if (sel.start.line !== sel.end.line) {
  //     throw new Error("align: selection must be on a single line");
  //   }
  //   col = sel.start.line === last_line ? col + 1 : 0;
  ...
  return edit((builder, selections) => {
    for (let i = 0, len = selections.length; i < len; i++) {
      ...
      builder.insert(
        selection.start,
        fill.repeat(startChar - selection.start.character),
      );
    }
  });
}
 ...
 * before the first character of each selection.
 *
 * @keys `c-&` (helix: normal), `c-&` (helix: select)
 */
export function alignByOffet({
  _,
  count,
  document,
  fallback,
}: CommandArguments<{ fallback?: string }>) {
  const startChar = _.selections.reduce(
    (max, sel) => (sel.start.character > max ? sel.start.character : max),
    0,
  );
    ...
    for (let i = 0, len = selections.length; i < len; i++) {
      ...
      const fill =
        document.lineAt(selection.start.line).text[
          selection.start.character - count
        ] ?? fallback;
      builder.insert(
        selection.start,
        fill.repeat(startChar - selection.start.character),
 ...
 *
 * @keys `s-a-o` (kakoune: normal), `s-a-o` (kakoune: select)
 *
 * @commands
 *
 * | Title                                      | Identifier             | Keybinding              | Commands                                                                          |
 * | ------------------------------------------ | ---------------------- | ----------------------- | --------------------------------------------------------------------------------- |
 * | Insert new line above and switch to insert | `newLine.above.insert` | `s-o` (helix: normal), `s-o` (helix: normal) | `[".edit.newLine.above", { shift: "select" }], [".modes.insert.before", { ... }]` |
 */
export function newLine_above({
  _,
  repetitions,
  shift,
}: CommandArguments<{ shift?: Shift }>) {
  if (shift === Shift.Select) {
    return insertLinesNativelyAndCopySelections(
      _,
      repetitions,
 ...
 * keep the current selections.
 *
 * @keys `a-o` (kakoune: normal), `a-o` (kakoune: select)
 *
 * @commands
 *
 * | Title                                      | Identifier             | Keybinding                               | Commands                                                                          |
 * | ------------------------------------------ | ---------------------- | ---------------------------------------- | --------------------------------------------------------------------------------- |
 * | Insert new line below and switch to insert | `newLine.below.insert` | `o` (helix: normal), `o` (helix: normal) | `[".edit.newLine.below", { shift: "select" }], [".modes.insert.before", { ... }]` |
 */
...
function prepareSelectionForLineInsertion(
  _: number,
  ...
  if (selection.active.line !== activeLine) {
    return new vscode.Selection(
      selection.anchor,
      selection.active.with(activeLine),
    );
  }
...
async function insertLinesNativelyAndCopySelections(
  ...
  for (let selection of _.selections) {
    ...
    for (let i = repetitions - 1; i > 0; i--) {
      selections.push(
        Selections.fromAnchorActive(anchor.translate(i), active.translate(i)),
      );
    }
