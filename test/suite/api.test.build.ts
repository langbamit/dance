...
export function build(builder: Builder) {
  const modules = builder.getApiModules();

  return (
    modules
      .map((module) => {
        const examples = module.functions.flatMap((f) =>
          f.examples.map((example) => {
      ...
      suite(${JSON.stringify(module.path.replace(/^dance/, "."))}, function () {
        ${examples
          .map((example) => {
            ...
            const examplesForFunction = examplesPerFunction.get(
            ...
            if (examplesForFunction === undefined) {
              ...
              examplesPerFunction.set(
                example.functionName,
                examplesForFunction + 1,
              );
            }
            ...
            for (const name of ["before", "after"] as const) {
              ...
              if (code !== undefined) {
                decls.push(
                  `${name} = ExpectedDocument.parseIndented(${stringifyExpectedDocument(
                    code,
                    12,
                  )})`,
                );
