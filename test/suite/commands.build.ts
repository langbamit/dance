          ...
          .map((test) => {
            ...
            test("${test.titleParts.join(" > ")}", async function () {
              ...
              await ExpectedDocument.apply(editor, ${replaceInTest(
                test.comesAfter,
                stringifyExpectedDocument(test.comesAfter.code, 4),
              )});
              ...
              ExpectedDocument.assertEquals(editor, "./test/suite/commands/${path.basename(
                file,
              )}:${test.line + 1}:1", ${replaceInTest(
              test,
              stringifyExpectedDocument(test.code, 4),
            )});
            });`;
          })
          .join("\n")}
...
function parseMarkdownTests(contents: string) {
  ...
  for (const [
    ...
    operationsText,
    after,
  ] of execAll(re, contents)) {
    const title = badTitle.replace(/\s/g, "-");

    assert(
      !all.has(title),
      `document state "${title}" is defined multiple times`,
    );
    ...
    if (titleParts.length !== nesting && titleParts.length <= 6) {
      console.warn(
        `section "${title}" has ${titleParts} parts but a nesting of ${nesting}`,
      );
    }
    ...
    if (comesAfterTitle === undefined) {
      ...
      const flags = execAll(/^> *(.+)$/gm, operationsText).map(
          ([_, flag]) => flag,
        ),
    ...
    assert(
      comesAfter !== undefined,
      `test "${title}" depends on unknown test "${comesAfterTitle}"`,
    );
...
function stringifyOperations(test: Test) {
  ...
  if (test.behavior === "character") {
    text +=
      `await executeCommand("dance.dev.setSelectionBehavior", ` +
  ...
  for (let i = 0; i < operations.length; i++) {
    ...
    while (
      ...
      operations[i + 1].command.startsWith("type:")
    ) {
      const text = operations[++i].command[5];

      promises.push(
        `new Promise((resolve) => setTimeout(() => executeCommand("type", { text: ${JSON.stringify(
          text,
        )} }).then(resolve), ${promises.length * 20}))`,
      );
    }
