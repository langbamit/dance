...
export async function run(testsRoot: string) {
  ...
  const currentFile = (process.env["CURRENT_FILE"] ?? "").replace(/\\/g, "/"),
    currentLine = process.env["CURRENT_LINE"]
      ? +process.env["CURRENT_LINE"] - 1
      : undefined,
  ...
  if (currentFile === "test/README.md") {
    ...
    if (currentFile.endsWith(".md")) {
      files = [
        path.join("commands", path.basename(currentFile, ".md") + ".test.js"),
      ];
      ...
      files = [
        path.join("commands", path.basename(currentFile, ".ts") + ".js"),
      ];
    }
